...
export async function savelog(msg){
  ...
  const log = await helpers.query("INSERT INTO `logs` (`userId`, `sessionId`, `requestId`, `logPath`, `timestamp`) VALUES (?, ?, ?, ?, ?)", [msg.userId, msg.sessionId, msg.requestId, "String " + format(msg.logPath), Date.now()]);
